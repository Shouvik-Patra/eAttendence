...
const InformalAttendance = props => {
  ...
  function onClockIn(capturedimage) {
    ...
    formData.append('remarks', 'clock in');
    formData.append('user_id', props.route.params.workerId);
    ...
    connectionrequest()
      .then(() => {
        dispatch(InformalclockinRequest(formData));
      })
      .catch(err => {
        console.log(err);
  ...
  function onClockOut(capturedimage) {
    ...
    connectionrequest()
      .then(() => {
        dispatch(InformalclockoutRequest(formData));
      })
      .catch(err => {
        console.log(err);
        showErrorAlert('Please connect to internet');
  ...
  // Navigate back to InformalHome with refresh flag
  const navigateBackToHome = () => {
    setLoading(false);
    setIsSubmitting(false);

    // Navigate back with refresh parameter
    props?.navigation.navigate('InformalAttendanceBottomTab', {
      screen: 'InformalHome',
      params: {
        shouldRefresh: true, // This will trigger the API call
  ...
  if (status == '' || InformalProfileReducer.status != status) {
    switch (InformalProfileReducer.status) {
      case 'InformalProfile/InformalclockinRequest':
        status = InformalProfileReducer.status;
        break;
      case 'InformalProfile/InformalclockinSuccess':
        status = InformalProfileReducer.status;
          connectionrequest()
          .then(() => {
        ...
        navigateBackToHome();
        break;
      case 'InformalProfile/InformalclockinFailure':
        status = InformalProfileReducer.status;
        navigateBackToHome();
        break;

      case 'InformalProfile/InformalclockoutRequest':
        status = InformalProfileReducer.status;
        break;
      case 'InformalProfile/InformalclockoutSuccess':
        status = InformalProfileReducer.status;
        connectionrequest()
          .then(() => {
            dispatch(informalUserDetailsRequest());
        ...
        navigateBackToHome();
        break;
      case 'InformalProfile/InformalclockoutFailure':
        status = InformalProfileReducer.status;
        showErrorAlert('Clock Out failed due to Network issue, Try again!');
        navigateBackToHome();
        break;
    }
  }
...
export default InformalAttendance;
